  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        ...
        const movie = data.movies.find(movie => movie.id === parseInt(idmovie));
    ...
    const fetchMovieData = async () => {
      try {
        const response = await fetch(`https://api.themoviedb.org/3/movie/${idmovie}/videos?language=en-US`, options);
        const data = await response.json();
  ...
  const handleDislike = (userId, movieId) => {
    return (event) => {
      ...
      axios
        ...
        .then((response) => {
          console.log('Film ajouté à la liste disliked_movies de l\'utilisateur avec succès');
        })
        .catch((error) => {
          console.error('Erreur lors de l\'ajout du film à la liste liked_movies de l\'utilisateur', error);
        });
