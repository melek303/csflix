...
const TitleCardsRecom = ({ title, category, query, iduser }) => {
  ...
  const updateRecommendedMovies = (likedMovies, dislikedMovies) => {
    ...
    const likedMoviesObjects = movies.filter(movie => likedMovies.includes(movie.id));
    const similarities = movies
      .filter(movie => !likedMovies.includes(movie.id) && !dislikedMovies.includes(movie.id))
      .map(movie => ({
        ...movie,
    ...
    const fetchMovies = async () => {
      try {
        ...
        const parsedMovies = data.movies.map(movie => ({
          ...movie,
          genre_ids: Array.isArray(movie.genre_ids) ? movie.genre_ids : movie.genre_ids.split(',').map(Number)
        }));
  ...
  return (
    <div className='title-cards'>
      ...
      <div className="card-list" ref={cardsRef}>
        {recommendedMovies.length > 0 ? recommendedMovies.map((movie, index) => (
